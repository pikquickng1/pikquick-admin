...
export function useRunnerActions() {
  ...
  const suspendAccount = async (id: string) => {
    try {
      setLoading(true);
      setError(null);
      await runnerApi.suspendRunner(id);
      return true;
    } catch (err) {
  ...
  const activateAccount = async (id: string) => {
    try {
      setLoading(true);
      setError(null);
      await runnerApi.activateRunner(id);
      return true;
    } catch (err) {
  ...
  const adjustWallet = async (id: string, type: "debit" | "credit", amount: number) => {
    try {
      setLoading(true);
      setError(null);
      await runnerApi.adjustWallet(id, type, amount);
      return true;
    } catch (err) {
  ...
  const resetPassword = async (id: string) => {
    try {
      setLoading(true);
      setError(null);
      await runnerApi.resetPassword(id);
      return true;
    } catch (err) {
  ...
  const sendMessage = async (id: string, subject: string, message: string) => {
    try {
      setLoading(true);
      setError(null);
      await runnerApi.sendMessage(id, subject, message);
      return true;
    } catch (err) {
